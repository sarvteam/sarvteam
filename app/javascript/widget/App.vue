...
export default {
  ...
  mounted() {
    const { websiteToken, locale, widgetColor } = window.sarvteamWebChannel;
  ...
  methods: {
    ...
    setLocale(localeWithVariation) {
      ...
      const { enabledLanguages } = window.sarvteamWebChannel;
    ...
    registerCampaignEvents() {
      ...
      bus.$on('execute-campaign', campaignDetails => {
        ...
        const { websiteToken } = window.sarvteamWebChannel;
