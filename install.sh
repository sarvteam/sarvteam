...
# Description: Install and manage a SarvTeam installation.
# OS: Ubuntu 20.04 LTS
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/sarvteam-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for SarvTeam (e.g., sarvteam.domain.com): ' domain_name
...
serve SarvTeam at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "sarvteam"; then
    adduser --disabled-login --gecos "" sarvteam
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/sarvteam/config
  file="/opt/sarvteam/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/sarvteam/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create sarvteam db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    CREATE USER sarvteam CREATEDB;
    ALTER USER sarvteam PASSWORD :'pass';
    ALTER ROLE sarvteam SUPERUSER;
...
function setup_sarvteam() {
  ...
  sed -i -e '/POSTGRES_USERNAME/ s/=.*/=sarvteam/' .env
...
function run_db_migrations(){
  sudo -i -u sarvteam << EOF
  cd sarvteam
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:sarvteam_prepare
EOF
}

##############################################################################
# Setup SarvTeam systemd services and cwctl CLI
# Globals:
#   None
# Arguments:
#   None
...
function configure_systemd_services() {
  cp /home/sarvteam/sarvteam/deployment/sarvteam-web.1.service /etc/systemd/system/sarvteam-web.1.service
  cp /home/sarvteam/sarvteam/deployment/sarvteam-worker.1.service /etc/systemd/system/sarvteam-worker.1.service
  cp /home/sarvteam/sarvteam/deployment/sarvteam.target /etc/systemd/system/sarvteam.target

  cp /home/sarvteam/sarvteam/deployment/sarvteam /etc/sudoers.d/sarvteam
  cp /home/sarvteam/sarvteam/deployment/setup_20.04.sh /usr/local/bin/cwctl
  chmod +x /usr/local/bin/cwctl

  systemctl enable sarvteam.target
  systemctl start sarvteam.target
}
...
#  wget https://raw.githubusercontent.com/sarvteam/sarvteam/develop/deployment/nginx_sarvteam.conf
  cp nginx_sarvteam.conf /etc/nginx/sites-available/nginx_sarvteam.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/sarvteam.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_sarvteam.conf
  ln -s /etc/nginx/sites-available/nginx_sarvteam.conf /etc/nginx/sites-enabled/nginx_sarvteam.conf
  systemctl restart nginx
  sudo -i -u sarvteam << EOF
  cd sarvteam
  sed -i "s/http:\/\/0.0.0.0:3000/https:\/\/$domain_name/g" .env
EOF
  systemctl restart sarvteam.target
}
...
function setup_logging() {
  touch /var/log/sarvteam-setup.log
  LOG_FILE="/var/log/sarvteam-setup.log"
}
...
***************************************************************************
Woot! Woot!! SarvTeam server installation is complete.
The server will be accessible at https://$domain_name

Join the community at https://sarv.com/community?utm_source=cwctl
***************************************************************************
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new SarvTeam CLI tool to manage your installation.'
...
#  CW_VERSION=$(curl -s https://app.sarv.com/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
  CW_VERSION="2.18.0"
}
...
***************************************************************************
              SarvTeam Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/sarvteam-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for SarvTeam?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing SarvTeam. This takes a long while."
  setup_sarvteam &>> "${LOG_FILE}"
...
https://www.sarv.com/docs/deployment/deploy-sarvteam-in-linux-vm?utm_source=cwctl
...
function get_console() {
  sudo -i -u sarvteam bash -c " cd sarvteam && RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your SarvTeam installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of SarvTeam
  -I                        Install SarvTeam from a git branch
  -u, --upgrade             Upgrade SarvTeam to the latest stable version
...
Management:
  ...
  -l, --logs                View logs from SarvTeam. Supported values include web/worker.
  -r, --restart             Restart SarvTeam server
...
Report bugs at https://github.com/sarvteam/sarvteam/issues
Get help, https://sarv.com/community?utm_source=cwctl

EOF
}

##############################################################################
# Get SarvTeam web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u sarvteam-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u sarvteam-web.1.service -f
  fi
}
...
function upgrade_redis() {
  echo "Upgrading Redis to v7+ for Rails 7 support(SarvTeam v2.17+)"
  curl -fsSL https://packages.redis.io/gpg | sudo gpg --dearmor -o /usr/share/keyrings/redis-archive-keyring.gpg
  echo "deb [signed-by=/usr/share/keyrings/redis-archive-keyring.gpg] https://packages.redis.io/deb $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/redis.list
  apt update -y
...
function upgrade() {
  get_cw_version
  echo "Upgrading SarvTeam to v$CW_VERSION"
  sleep 3
  upgrade_prereq
  upgrade_redis
  sudo -i -u sarvteam << "EOF"

  # Navigate to the SarvTeam directory
  cd sarvteam
...
EOF
  ...
  # Restart the sarvteam server
  systemctl restart sarvteam.target

}

##############################################################################
# Restart SarvTeam server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  systemctl restart sarvteam.target
  systemctl status sarvteam.target
}
